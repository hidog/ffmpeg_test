...
public:
    NvEncoderInitParam(const char *szParam = "", std::function<void(NV_ENC_INITIALIZE_PARAMS *pParams)> *pfuncInit = NULL, bool _bLowLatency = false) 
    ...
    virtual void SetInitParams(NV_ENC_INITIALIZE_PARAMS *pParams, NV_ENC_BUFFER_FORMAT eBufferFormat)
    {
        ...
        for (unsigned i = 0; i < tokens.size(); i++)
        {
            if (
                ...
                tokens[i] == "-fpsn"       && ++i != tokens.size() && ParseInt("-fpsn",           tokens[i], &pParams->frameRateNum)                                                      ||
                tokens[i] == "-fpsd"       && ++i != tokens.size() && ParseInt("-fpsd",           tokens[i], &pParams->frameRateDen)                                                      ||
