...
static AVFrame *get_video_frame(OutputStream *ost)
{
    ...
    * internally; make sure we do not overwrite it here */
    ...
    if( !ost->sws_ctx )
    {
        ost->sws_ctx = sws_getContext( 1280, 720, AV_PIX_FMT_BGRA,
    ...
    sws_scale( ost->sws_ctx, ptr, linesize, 0, 720, video_dst_data, video_dst_linesize );

    memcpy( ost->frame->data[0], video_dst_data[0], video_dst_linesize[0]*1080 );
    memcpy( ost->frame->data[1], video_dst_data[1], video_dst_linesize[1]*1080/4 );
    memcpy( ost->frame->data[2], video_dst_data[2], video_dst_linesize[2]*1080/4 );
...
int muxing()
{
    video_dst_bufsize   =   av_image_alloc( video_dst_data, video_dst_linesize, 1280, 720, AV_PIX_FMT_YUV420P, 1 );
